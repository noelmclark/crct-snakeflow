# simple awk script that picks out the members of a given
# scatter group.  If it picks out none, then it throws an error.

# use -v on invocation to set the value of scat to the scatter_group id
...
END {
	if(n==0) {
		print "Didn't find any elements in scat group", scat ". Bailing out!" > "/dev/stderr"
		exit(1);
	}
}